...
function isExcluded(name){ const s=name.toLowerCase(); return s.includes('mega')||s.includes('gmax')||s.includes('gigantamax')||s.includes('dynamax')|| (s.includes('dyna')&&s.includes('max')); }
export default function App(){
  ...
  function shuffle(arr){ const a=[...arr]; for(let i=a.length-1;i>0;i--){ const r=Math.floor(Math.random()*(i+1)); [a[i],a[r]]=[a[r],a[i]] } return a; }
  ...
  function startVoting(resume=false){
    ...
    if(resume && resumeRef.current){
      ...
      const pool = resumeRef.current.deck.map(n=>lookup[n]).filter(Boolean);
  ...
  useEffect(()=>{
    if(flow==='voting' && deck.length>0){
      ...
      try{ const saves=JSON.parse(localStorage.getItem('pf_manual_saves')||'[]'); const auto={name:'Auto-save',ts:new Date().toISOString(), session}; const others=saves.filter(s=>s.name!=='Auto-save'); localStorage.setItem('pf_manual_saves', JSON.stringify([auto,...others].slice(0,20))); }catch(e){} 
    }
  },[deck,index,flow]);
  ...
  function manualSavePrompt(){ const name=prompt('Save name (optional)'); if(name===null) return; saveManual(name); alert('Saved.'); }
  ...
  return (
    <div className='app'>
      ...
      <Header startVoting={startVoting} savedResults={savedResults} setFlow={setFlow} saveManual={manualSavePrompt} openLoadModal={openLoadModal} resumeAvailable={Boolean(resumeRef.current)} />
      <main className='main'>
