      ...
      <ul className='savedList'>
        {savedResults.map((s,i)=>(
          <li key={i}>
            ...
            <div><button onClick={()=>{ const data=JSON.stringify(s,null,2); const blob=new Blob([data],{type:'application/json'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`pokefavor-result-${i}.json`; a.click(); URL.revokeObjectURL(url); }}>Export</button></div>
          </li>
        ))}
      </ul>
